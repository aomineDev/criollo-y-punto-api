...
public class ReserveController {
	...
	@GetMapping("/{id}")
	public ResponseEntity<Reserve> getReserve(@PathVariable Integer id) {
	...
	public ResponseEntity<Reserve> createReserve(@RequestBody Reserve reserve) {
		return ResponseEntity.status(HttpStatus.CREATED).body(service.save(reserve));
	}

	@PutMapping("/{id}")
	public ResponseEntity<Reserve> updateReserve(@PathVariable Integer id,
			@RequestBody Reserve reserve) {
		return service.update(id, reserve)
				.map(ResponseEntity::ok)
				.orElseGet(() -> ResponseEntity.notFound().build());
	}

	@DeleteMapping("/{id}")
	public ResponseEntity<Void> deleteReserve(@PathVariable Integer id) {
		service.delete(id);
